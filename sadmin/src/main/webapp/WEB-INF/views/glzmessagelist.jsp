...
<form  method="GET" id="exception_form" name="exception_form" action="glzmessage.do">
    <table class="table table-striped">
        ...
        <tr class="span6">
            ...
            topic :<select name="topic" style="width:120px">
                       ...
                       <option value="invite" <c:if test="${'invite' eq topic}">selected</c:if> >invite</option>
                       <option value="resume" <c:if test="${'resume' eq topic}">selected</c:if> >resume</option>
                   </select>
                   &nbsp;&nbsp;
            ...
            <!--
            <a class="btn btn-info" href="#postMessage" data-toggle="modal" style="margin-left: 30px;" onclick="javascript: return postMessage()">
...
<div class="pagination pagination-centered" style="display: ${morePage=="1"?"block":"none"}">
    <ul>
        <c:choose>
            ...
            <c:otherwise><li><a href="glzmessage.do?page=${current-1}&date=${date}&startTime=${startTime}&endTime=${endTime}&topic=${topic}&morePage=${morePage}&message=${message}">Prev</a></li></c:otherwise>
        </c:choose>
        <c:forEach items="${num }" var="n">
            <c:choose>
                <c:when test="${n==current}">
                    <li class="active">
                        <a href="glzmessage.do?page=${n}&date=${date}&startTime=${startTime}&endTime=${endTime}&topic=${topic}&morePage=${morePage}&message=${message}" >${n }</a>
                    </li>
                </c:when>
                <c:otherwise>
                    <li><a href="glzmessage.do?page=${n}&date=${date}&startTime=${startTime}&endTime=${endTime}&topic=${topic}&morePage=${morePage}&message=${message}" >${n }</a></li>
                </c:otherwise>
            </c:choose>
        </c:forEach>
        <c:choose>
            <c:when test="${current==total || total==0}"><li class="disabled"><a href="#">Next</a></li></c:when>
            <c:otherwise><li><a href="glzmessage.do?page=${current+1}&date=${date}&startTime=${startTime}&endTime=${endTime}&topic=${topic}&morePage=${morePage}&message=${message}">Next</a></li></c:otherwise>
        </c:choose>
    </ul>
</div>
...
<div class="modal hide fade" id="postMessage">
    ...
    <div class="modal-footer">
        <form method="post" action="glz_message_post.do">
            <fieldset>

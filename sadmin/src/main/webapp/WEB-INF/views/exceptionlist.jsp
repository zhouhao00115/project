...
%>
<form  method="GET" id="exception_form" name="exception_form" action="exception.do">
	  	  ...
	  	  <tr class="span10">
              ...
              &nbsp;
	  	  	  <input class="btn btn-primary" type="submit" value="search"
                     onclick="return searchException('exception_form')"/>
              &nbsp;&nbsp;
              <input class="btn btn-primary" type="button" value="export"
                     onclick="return exportException('exception_form')"/>
              &nbsp;
              ...
              <input type="hidden" name="exportPage" value="${current}"/>
	  	  	  <a class="btn btn-danger" href="#delModal" data-toggle="modal" style="margin-left: 10px;" onclick="return exceptiondel()">
                 Delete
              </a>
	  	  </tr>
	  	  <tr>
 ...
 <div class="pagination pagination-centered">
  <ul>
  	<c:choose>
  		...
  		<c:otherwise><li><a href="exception.do?page=${current-1 }&description=${descripiton}&startTime=${startTime}&endTime=${endTime}&exception=${exception}" >Prev</a></li></c:otherwise>
  	</c:choose>
	  <c:forEach items="${num }" var="n">
	  	<c:choose>
	  		<c:when test="${n==current }">
	  			<li class="active">
				  <a href="exception.do?page=${n }&description=${description}&startTime=${startTime}&endTime=${endTime}&exception=${exception}" >${n }</a>
				</li>
	  		</c:when>
	  		<c:otherwise>
	  			<li><a href="exception.do?page=${n }&description=${description}&startTime=${startTime}&endTime=${endTime}&exception=${exception}" >${n }</a></li>
	  		</c:otherwise>
	  	</c:choose>
	  </c:forEach>
	<c:choose>
		<c:when test="${current==total || total==0 }"><li class="disabled"><a href="#">Next</a></li></c:when>
		<c:otherwise><li><a href="exception.do?page=${current+1 }&description=${descripiton}&startTime=${startTime}&endTime=${endTime}&exception=${exception}" >Next</a></li></c:otherwise>
	</c:choose>
  </ul>
</div>
